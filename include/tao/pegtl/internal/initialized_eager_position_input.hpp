...
#ifndef TAO_PEGTL_INTERNAL_INITIALIZED_EAGER_POSITION_INPUT_HPP
#define TAO_PEGTL_INTERNAL_INITIALIZED_EAGER_POSITION_INPUT_HPP
...
namespace tao::pegtl::internal
{
   ...
   class initialized_eager_position_input
      : public Input
   {
   public:
      ...
      explicit initialized_eager_position_input( Position&& p, As&&... as )  // noexcept( auto )
         : Input( std::forward< As >( as )... ),
           m_position( std::move( p ) )
      {}

      template< typename... As >
      explicit initialized_eager_position_input( const Position& p, As&&... as )  // noexcept( auto )
         : Input( std::forward< As >( as )... ),
           m_position( p )
      {}
      ...
      [[nodiscard]] rewind_guard< M, initialized_eager_position_input > make_rewind_guard() noexcept
      {
         return rewind_guard< M, initialized_eager_position_input >( this );
      }
