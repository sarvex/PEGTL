...
namespace tao::pegtl::internal
{
   ...
   struct rematch< Head, Rule, Rules... >
   {
      ...
      // TODO: A more succinct implementation based on the same pattern as in 3.x (if we can determine and instantiate an appropriate memory_input)?
      ...
      [[nodiscard]] static bool match( ParseInput& in, States&&... st )
      {
         ...
         if( Control< Head >::template match< A, rewind_mode::active, Action, Control >( in, st... ) ) {
            ...
            try {
               ...
               const bool r = ( ( m.repeatable_rewind(), Control< Rule >::template match< A, rewind_mode::active, Action, Control >( in, st... ) ) && ... && ( m.repeatable_rewind(), Control< Rules >::template match< A, rewind_mode::active, Action, Control >( in, st... ) ) );
